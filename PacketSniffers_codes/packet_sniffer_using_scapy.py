...
def process_packet(packet, network_metrics):
    ...
    if IP in packet:
        if TCP in packet:
            process_tcp_packet(packet , current_time, network_metrics)
        elif UDP in packet:
            process_udp_packet(packet, current_time, network_metrics)
        else:
            process_other_packet(packet, current_time, network_metrics)
...
def main():
    ...
    if selected_iface:
        ...
        sniff(iface=selected_iface, prn= lambda pkt : process_packet(pkt, network_metrics), store=0, count=5)
    else:
        print("Sniffing packets from default interface:")
        sniff(prn=lambda pkt : process_packet(pkt, network_metrics), store=0, count=5)
