...
@app.route('/check-anomalies', methods=['GET'])
@cross_origin()
def check_anomalies():
    ...
    app.logger.info(f"Connecting to Kafka: Topic={topic}, Broker={broker}")
    ...
    try:
        consumer = KafkaConsumer(
            ...
            value_deserializer=lambda m: json.loads(m.decode('utf-8'))  # Auto-decode JSON messages
        )
    except Exception as e:
        app.logger.error(f"Kafka consumer initialization failed: {e}")
        return jsonify({"error": f"Error connecting to Kafka: {str(e)}"}), 500
    ...
    try:
        for message in consumer:
            ...
            data = message.value  # Since we already decoded JSON
            app.logger.info(f"Received Kafka message: {data}")

            if data.get("status") == "anomaly":  # Adjust based on actual Kafka message format
                anomalies.append(data)

            if len(anomalies) > 10:  # Return response after collecting some anomalies
                break

    except Exception as e:
        app.logger.error(f"Kafka streaming error: {e}")
        return jsonify({"error": f"Streaming error: {str(e)}"}), 500
    finally:
        try:
            consumer.close()
            app.logger.info("Kafka consumer closed successfully.")
        except Exception as close_error:
            app.logger.error(f"Error closing Kafka consumer: {close_error}")

    return jsonify({"status": "anomaly_detected" if anomalies else "no_anomalies", "anomalies": anomalies})
