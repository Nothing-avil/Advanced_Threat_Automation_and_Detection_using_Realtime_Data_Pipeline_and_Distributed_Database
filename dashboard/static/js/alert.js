window.onload = () => {
  init("System is healthy. No anomalies detected.");
  setInterval(() => {
    checkForAnomalies((statusMessage) => {
      init(statusMessage);  
    });
  }, 10000);
};
...
function init(initialText) {
  ...
  currentTextAnimation = createTextAnimation(initialText);
  currentTextAnimation.position.y = -40;
  root.scene.add(currentTextAnimation);
...
function createTextAnimation(text) {
  const geometry = generateTextGeometry(text, {
    size: 5,
    height: 0,
    font: "droid sans",
    weight: "bold",
...
function checkForAnomalies(callback) {
  fetch("/check-anomalies")
    .then((response) => response.json())
    .then((data) => {
      console.log(data);
      let text = "System is healthy. No anomalies detected.";
      if (data.status === "anomaly_detected") {
        document.getElementById("anomaly-alert").style.display = "block";
        document.getElementById("secured_system").style.display = "none";
        ...
        document.getElementById("secured_system").style.display = "block";
...
function updateTextAnimation(newText) {
  if (currentTextAnimation && currentTextAnimation.parent) {
    currentTextAnimation.parent.remove(currentTextAnimation);
  }
  currentTextAnimation = createTextAnimation(newText);
  currentTextAnimation.position.y = -40;

  const rootElement = document.getElementById("three-container");
  if (rootElement && rootElement.scene) {
    rootElement.scene.add(currentTextAnimation);
  }
}
