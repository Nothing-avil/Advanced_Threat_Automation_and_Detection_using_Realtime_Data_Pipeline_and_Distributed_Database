...
function addMarker(lat, lng, label) {
    ...
    marker.on('mouseover', () => {
        ...
        popup.innerHTML = `Your Location: ${label}`;
        document.body.appendChild(popup);

        function updatePopupPosition(mouseEvent) {
            popup.style.left = `${mouseEvent.clientX + 10}px`;
            popup.style.top = `${mouseEvent.clientY + 10}px`;
        }

        renderer.domElement.addEventListener('mousemove', updatePopupPosition);

        marker.on('mouseout', () => {
            document.body.removeChild(popup);
            renderer.domElement.removeEventListener('mousemove', updatePopupPosition);
        });
    });
}
